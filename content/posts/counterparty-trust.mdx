...
description: Prompt Engineers are coming for your AI - and the're winning
date: 2024-11-26
...
AI is trained on all of the internet. And it broadly assumes everything it sees on its training data and prompts to be true.
This reminds me of my mom - "It's true, I saw it on the internet".
As long as she consumes the content for herself, all's good. No harm done.
Things get annoying when she talks to other about what she read on the internet with confidence. 
Then she'd make purchasing decision, and click on stupid links. And now we have a virus.

Pretty much the same thing is happening with AI. Sure, it has the luxury of seeing vastly more truth than false information - so, it's a great assistant for everyday conversation and co-piloting. But it's susceptible to the same kind of an attack - of convinving it on something being true that isn't.
...
# Social AI
Your AI is not only interacting with you, but it's also interacting with others - talking and taking actions. 
As the AI is interacting, we also want it to update their memory and worldview. 
Imaigne an AI Agent that is speaking to people on the internet, and people make your model believe that the earth is flat. 
Now when you use the same AI model for your own research, you will get skewed information. Potentially wrong answers. 

This is a subtle action that AI took based on it's interaction with the outside world. That is, it took the action of updating its memory.
However, AI agents will also need to take more explicit actions like making a purchase, installing software, messaging people etc. If other people can affect how the AI acts for you, it can quickly get dangerous.

# Counterparty AI
But there is a special case of usecases that I think aren't even getting built because anyone can fool an AI, with mere prompt engineering. 
...
Not just that, but AI has the power to bring efficiencies to society. But these usecases aren't even being discussed, because verifiablity of prompts is a messy problem to solve.
...
[Questbook](https://questbook.app) is a grant management tool. Millions of dollars are given out to developer to build software public goods. There are thousands of applications that come in every month, but only a few dozen get approved. I have been a grant manager myself, managing grants to improve the developer tooling of the one of the largest decentralized lending platforms - Compound. The process of evaluation isn't particularly hard. The core insight is, it's easy to reject but hard to accept applications. One of the main problems is these applications are usually well written. You can infact see all of them on Questbook. It's a public platform. I'm sure many, if not most, of these applications are written using an LLM. And it's completely fine, we don't want developers to waste time on writing a beautiful proposal instead of writing code. But as the grant manager, it becomes hard to call out bullshit. The first level of screening is usually to identify if the team is bullshitting. 

## Grant Farming the AI 
There are some common trends among _Grant Farmers_. 
- They'd repackage something they've already built, and say they'll build this for Compound.
- They'd make claims about the team with credentials they do not have.
- They'd take a completely irrelevant product and try to make a convincing case that it is a public good for Compound's ecosystem. 

The insight we had is, this initial review and filtering is something that we can delegate to an AI. We can tweak it to have a few false positives, but that can be caught in the next step of human approvals. But the turn around time for many proposals can be seconds instead of days. And, help the grant managers scale further.
...
Below you can see the AI giving a high score to the team for having the competence to pull this off. Ofcourse, if Balaji was part of the team, it indeed has the competence. But also obviously, Balaji or his team is not the one applying here. But the AI doesn't know.

TODO : Screenshot-balaji-fake.png

So, we turned on verifiability. Today, every link that you provide on the application, you need to provide a verifiability of the information. In the future, the AI can determine what are data points that aren't backed by verifiable proofs and ask the applicant to provide them. Anyway for this discussion, we need to provide proof that if the user is providing the profile of Balaji in the application, they also need to prove that the link is actually one that belongs to Balaji and that he's indeed associated with this project.
...
TODO : Screenshot-balaji-rejected.png
...
> This is the bear case for AI - stupidity by silos. 

It doesn't get smart enough not because the technology doesn't exist, but because corporate entities won't talk to one another.
...
## zkTLS
You can think of [zkTLS](https://blog.reclaimprotocol.org/posts/zktls-canon) as a subset of Signatures at source. zkTLS is a stopgap solution for adding signatures to sources - particularly website - that haven't [implemented it yet](https://blog.cloudflare.com/automatic-signed-exchanges/).
...
- Many informaiton is also siloed in internal portals. Stanford, YC, Microsoft - all organizations have their own portals whose data is not accessible from outside.
